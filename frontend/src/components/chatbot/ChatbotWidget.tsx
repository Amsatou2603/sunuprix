      ...
      <AnimatePresence>
        {ouvert && (
          <motion.div
            ...
            className="mb-4 flex h-[min(32rem,80vh)] w-[23rem] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-3xl border border-[#00C49F]/20 bg-[#062118]/85 text-white shadow-2xl backdrop-blur-2xl"
          >
            {/* Header matching attached photo */}
            <div className="flex items-center justify-between border-b border-white/10 bg-[#041B13]/70 px-4 py-3.5 backdrop-blur-md">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-4">
              ...
              {messages.map((message, indice) => (
                ...
                >
                  ...
                  <div
                    className={`max-w-[82%] rounded-2xl px-4 py-3 text-xs leading-relaxed backdrop-blur-sm ${
                      message.role === "UTILISATEUR"
                        ? "bg-[#00C49F]/90 text-white font-medium shadow-md"
                        : "bg-[#0F382C]/80 text-emerald-50 border border-emerald-500/10 shadow-sm"
                    }`}
                  >
                    <p>{message.contenu}</p>
                  </div>
              ...
              {enCours && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5 rounded-2xl bg-[#0F382C]/80 px-4 py-3 backdrop-blur-sm">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {QUESTIONS_SUGGEREES.map((q) => (
                    <button
                      ...
                      className="rounded-full border border-[#00C49F]/30 bg-white/5 px-3 py-1 text-left text-[11px] font-medium text-emerald-200 backdrop-blur-sm transition hover:bg-white/10 hover:text-white"
                    >
                      {q}
                    </button>
                  ))}
            ...
            {erreur && (
              <p className="border-t border-red-900/50 bg-red-950/60 px-4 py-2 text-xs text-red-300 backdrop-blur-sm">
                {erreur}
              </p>
            )}

            {/* Input Bar */}
            <form onSubmit={envoyer} className="flex gap-2 border-t border-white/10 bg-[#041B13]/70 p-3 backdrop-blur-md">
              <input
                type="text"
                value={saisie}
                onChange={(e) => setSaisie(e.target.value)}
                placeholder="Posez votre question à SunuBot…"
                className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-xs text-white placeholder-emerald-100/40 backdrop-blur-sm focus:border-[#00C49F] focus:outline-none"
                disabled={enCours}
              />
              <button
                type="submit"
